...
#define ZIGBEE_TX_REQUEST_SIZE 0x19
...
//  write_eeprom(msb, 0x51);

  rx_serial();
  uint8_t lsb = s_val;
  buffer[sz++] = lsb;

//  write_eeprom(lsb, 0x98);
...
//     //write_eeprom(buffer[j], start++); 
   ...
   Serial.println(read_eeprom(0x98), HEX);
...
void setup1()
{
   ...
   while(true)
   {
//   //    b  = discovered_slave(slaves_64_address[slave_sz-1]);  
       uint8_t v = discovered_slave(slaves_64_address[slave_sz-1]);
       write_eeprom(v, 0x99);
       if(v == 1)
       {
          slave_sz--;
...
////
////           delay(5000);
////           b_1 = discovered_slave(slaves_64_address[slave_sz-1]);  // try again let the slave stabilize
////           if(b_1 & b)  // is the slave discovered again ?
////           {
////              slave_sz--;
////            
////           }
...
////       }
   ...
   //write_eeprom(slave_sz+100, 0x50);
   digitalWrite(13, LOW);
}
...
void setup0() 
{
  ...
  //Serial.begin(9600);
  //Serial.println("TEST");
  //uint8_t s = (0x10 + 0x01 + 0xFF + 0xFF + 0xEA + 0x21 + 0x09) & 0xFF ;
  //s = 0xFF - s;
  //Serial.println(s, HEX);
...
void loop1()
{
   ...
   if(slave_sz){ 
     ...
     {
        delay(5000);
        v = discovered_slave(slaves_64_address[slave_sz-1]);
        if(v)
        {
           slave_sz--;  
        }
