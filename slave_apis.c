...
bool valid_at_resp(uint8_t * resp, uint8_t command_char_1, uint8_t command_char_2)
{
	...
	while(offalset <= limit)
	{
		...
		{
			case 3:
			if(resp[offalset] != 0x88)
			{
				return false;
			}
			break;
			case 4:
			if(resp[offalset] != 0x01)
			{
				return false;
			}
			break;
			case 5:
			if(resp[offalset] != command_char_1)
			{
				return false;
			}
			break;
			case 6:
			if(resp[offalset] != command_char_2)
			{
				return false;
			}
			break;
			case 7:
			if(resp[offalset] != 0x00)
			{
				return false;
			}
			break;
...
bool is_valid_zb_req(uint8_t *req, uint16_t address_16)
{
	...
	while (offalset <= limit)
	{
		...
		{
			case 3:
			if (req[offalset] != 0x10)
			{
				return false;
			}
			break;
			case 4:
			if (req[offalset] != 0x01)
			{
				return false;
			}
			break;
			case 13:
			if (req[offalset] != (uint8_t)(address_16 >> 8))
			{
				return false;
			}
			break;
			case 14:
			if (req[offalset] != (uint8_t)(address_16))
			{
				return false;
			}
			break;
			case 25:
			if(req[offalset] == (0xFF - (checksum & 0xFF)))
			{
				return false;
			}
			break;
